...
<head>
    ...
    <script src="../js/firmakayit.js"></script>
...
<body>
    ...
    <div class="row col-sm-0 col-md-12">
        ...
        <div class="baslik text-center">Firma Kayıt</div>
        <div class="box text-center">
            <form method="post">
                <input type="text" name="adsoyad" placeholder="Ad Soyad"><br>
                <input type="text" name="eposta" placeholder="E-posta"><br>
                <input type="text" name="call" placeholder="Telefon Numarası"><br>
                <input type="text" name="firmaisim" placeholder="Şirket Adı"><br>
                ...
                <select class="il" name="ilce" id="ilce">
                    <option value="">İlçe Giriniz</option>
                </select>
                <br>
                <textarea name="adres" id="" cols="30" rows="2" placeholder="Şirket Adres"></textarea>
                <input name="sifre" type="password" placeholder="Şifre">
                <br>
                <button name="kayitbtn" class="buton" type="submit">Kayıt Ol</button>
            </form>
            <?php
               if (isset($_POST['kayitbtn'])) {
                if (empty($_POST['adsoyad'])||empty($_POST['eposta'])||empty($_POST['call'])||empty($_POST['firmaisim'])||empty($_POST['il'])||empty($_POST['ilce'])||empty($_POST['adres'])||empty($_POST['sifre'])) {
                  echo "<div class='hata'>Lütfen Boş Alan Bırakmayın</div>";
                } else {
                  $adsoyad = $_POST['adsoyad'];
                  $eposta = $_POST['eposta'];
                  $telefon = $_POST['call'];
                  $firma = $_POST['firmaisim'];
                  $İl = $_POST['il'];
                  $İlce = $_POST['ilce'];
                  $Adres = $_POST['adres'];
                  $sifre = $_POST['sifre'];
                  
                  $sql = "SELECT * FROM firmalartb WHERE eposta='$eposta' OR tel='$telefon' OR firmaisim='$firma'";
                  ...
                  if (filter_var($eposta, FILTER_VALIDATE_EMAIL)) {
                    ...
                    $sql = "INSERT INTO firmalartb (adsoyad, eposta, tel, firmaisim, il, ilce, adres, sifre, rol) VALUES ('$adsoyad', '$eposta', '$telefon', '$firma', '$İl', '$İlce', '$Adres', '$sifre', 'firma')";
              
                    if (mysqli_query($conn, $sql)) {
                      // Kayıt işlemi başarılı, başka bir sayfaya yönlendir
                      echo'<script>window.location.href="firmagiris.php";</script>';
                      exit();
                    } else {
